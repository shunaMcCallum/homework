...
def save(zombie_type):
    sql = "INSERT INTO zombie_types (name) VALUES (?) RETURNING id"
...
def select(id):
    sql = "SELECT * FROM zombie_types WHERE id = ?"
    values = [id]
...
def delete(id):
    sql = "DELETE FROM zombie_types WHERE id = ?"
    values = [id]
    run_sql(sql, values)


def update(zombie_type):
    sql = "UPDATE zombie_types SET name = ? WHERE id = ?"

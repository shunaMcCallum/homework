...
def save(zombie):
    sql = "INSERT INTO zombies (name, zombie_type_id) VALUES ( ?, ? ) RETURNING id"
...
def select(id):
    sql = "SELECT * FROM zombies WHERE id = ?"
    values = [id]
...
def delete(id):
    sql = "DELETE FROM zombies WHERE id = ?"
    values = [id]
    run_sql(sql, values)


def update(zombie):
    sql = "UPDATE zombies SET (name, zombie_type_id) = ( ?, ? ) WHERE id = ?"
...
def select_victims_of_zombie(id):
    victims = []
    sql = "SELECT humans.* FROM humans INNER JOIN bitings ON bitings.human_id = humans.id WHERE bitings.zombie_id = ?"
    values = [id]
    results = run_sql(sql, values)
    for result in results:

...
def save(biting):
    sql = "INSERT INTO bitings (human_id, zombie_id) VALUES ( ?, ? ) RETURNING id"
...
def select(id):
    sql = "SELECT * FROM bitings WHERE id = ?"
    values = [id]
...
def delete(id):
    sql = "DELETE FROM bitings WHERE id = ?"
    values = [id]
    run_sql(sql, values)


def update(biting):
    sql = "UPDATE bitings SET (human_id, zombie_id) = ( ?, ? ) WHERE id = ?"
